...
def _within_subject_experiment(name, windows_dataset, clf, n_epochs):
    f = open(f"./log/{name}-{time.time()}.txt", "w")
...
def bci2a_shallow_conv_net():
    ...
    ds = dataset_process.DatasetFromBraindecode('bci2a', subject_ids=None)
    ...
    clf = EEGClassifier(module=model,
                        criterion=CroppedLoss, criterion__loss_function=torch.nn.functional.nll_loss,
                        optimizer=torch.optim.AdamW, train_split=None, optimizer__lr=lr,
    ...
    _within_subject_experiment(name='ShallowConvNet', windows_dataset=windows_dataset, clf=clf, n_epochs=n_epochs)
...
def bci2a_eeg_net():
    ...
    _within_subject_experiment(name='EEGNet', windows_dataset=windows_dataset, clf=clf, n_epochs=n_epochs)


def bci2a_csp_lda():
    # get dataset
    ds = dataset_process.DatasetFromMoabb('bci2a')
